...
public class BSTReferenceBased {
   ...
   private Node insertRec( Node currRoot, String newKey, Object newItem ) throws Exception {
      ...
      if( currRoot == null ) {
         ...
         if( comparisonResult == 0 ) {
            ...
            throw new Exception( "BSTReferenceBased: insert failed because input key is already in BST!" );
         }
   ...
   //         Throws a BSTException (critical) if search fails because input key is not in BST.
   // Note: Recursive implementation.
   private Object searchRec( Node currRoot, String key ) throws Exception {
      // Check if input BST is empty.
      if( currRoot == null ) {
         // Input BST is empty, search failed, throw critical exception. 
         throw new Exception( "BSTReferenceBased: search failed because input key is not in BST!" );
      }
      else {
   ...
   // Output: Throws a BSTException (critical) if deletion fails because input key is not found in BST.
   // Note: Recursive implementation.
   private Node deleteRec( Node currRoot, String key ) throws Exception {
      // Check if input BST is empty.
      if( currRoot == null ) { 
         // Input BST is empty, input key not found, deletion failed, throw critical exception.
         throw new Exception( "BSTReferenceBased: delete failed because input key is not in BST!" );
      }
      else {
   ...
   public void printPreorder() throws RuntimeException {
      // Check if this BST is empty.
      if( this.numItems == 0 ) {
         // Ths BST is empty, print failed, throw non-critical exception.
         throw new RuntimeException( "BSTReferenceBased: print failed because BST is empty!" ); 
      }
      else {
   ...
   // Output: Throws a BSTRuntimeException (non-critical) if this BST is empty.
   public void printInorder() throws RuntimeException {
      // Check if this BST is empty.
      if( this.numItems == 0 ) {
         // Ths BST is empty, print failed, throw non-critical exception.
         throw new RuntimeException( "BSTReferenceBased: print failed because BST is empty!" ); 
   ...
   // Output: Throws a BSTRuntimeException (non-critical) if this BST is empty.
   public void printPostorder() throws RuntimeException {
      // Check if this BST is empty.
      if( this.numItems == 0 ) {
         // Ths BST is empty, print failed, throw non-critical exception.
         throw new RuntimeException( "BSTReferenceBased: print failed because BST is empty!" ); 
